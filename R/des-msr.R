...
#' DES with MSR (Median Success Rule) step-size adaptation rule 
#'
#' https://hal.inria.fr/hal-00801414v1/document
#'
#' params: as recomended in paper 
#'  c_param = 0.3
#'  d_param = 2 * (N - 1) / N) 
#'  j_index = floor(0.3 * lambda))
#' implementation: 
#'  check lines 372-375 

des_msr <- function(par, fn, ..., lower, upper, control = list()) {
  ...
  c_param     <- controlParam("c_param", 0.3)
  d_param     <- controlParam("d_param", 2 * (N - 1) / N)
  j_index     <- controlParam("j_index", floor(0.3 * lambda))
  s           <- controlParam("s", 0)
  ...
  log.Ft <- controlParam("diag.Ft", TRUE)
  ...
  while (counteval < budget) {
    ...
    while (counteval < budget && !stoptol) {
      ...
      ## Update Ft with MSR
      K_succ = sum(fitness <= old_fitness[j_index])
  ...
  res <- list(
    ...
    label = stringr::str_glue("des_msr-dp-{round(d_param, 2)}-cp-{round(c_param, 2)}"),
