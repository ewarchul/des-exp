...
#' DES with RSR (Rank Success Rule) step-size adaptation rule
#'
#' DOI: 10.1109/TEVC.2017.2765682

des_rsr <- function(par, fn, ..., lower, upper, control = list()) {
  ...
  while (counteval < budget) {
    ...
    while (counteval < budget && !stoptol) {
      ...
      ## Update Ft
      return(list(fit = fitness, oldfit = old_fitness))
      all_fitness_inx = sort(c(old_fitness, fitness))
